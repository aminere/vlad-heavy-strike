...
namespace shoot
{
	...
	MainFrame::MainFrame(const wxString& title, const wxSize& size /*= wxSize(800, 600)*/, ShootEditor* pCustomEditor /*= NULL*/)
	: wxFrame(NULL, wxID_ANY, title, wxDefaultPosition, size)	
	{
		...
		if(pCustomEditor)
		{
			ShootEditor::OverrideInstance(pCustomEditor);
		}
		else
		{
			ShootEditor::CreateInstance();
		}
	}
	...
	void MainFrame::Show()
	{
		...
		GetUIManager().AddPane(ToolbarManager::Instance()->GetMainToolbar(), wxAuiPaneInfo().Name(wxT("Main Toolbar")).Top().LeftDockable(false).RightDockable(false).CaptionVisible(false));
	...
	void MainFrame::OnInsert(wxCommandEvent& event)
	{
		...
		u32 typeIndex = event.GetId() - ID_Menu_Insert_FirstEntity;
		Entity* pEntity = NULL;
		...
		if(bIsTemplateInstance)
		{
			...
			{
				Object* pObject = ObjectManager::Instance()->Get(path.c_str());
		...
		else
		{
			...
			ParticleGenerator2D* pParticleGenerator2D = DYNAMIC_CAST(pEntity, ParticleGenerator2D);
			...
			else if(pParticleGenerator2D != NULL)
			{
				std::string path;
				if(EditorUtils::OpenFileDialog(this, path)) // TODO: set file filter
				{
					...
					GraphicComponent* pGraphic = snew GraphicComponent();
					pGraphic->SetMaterial(pMaterial);
					pGraphic->SetRenderingPass(GraphicComponent::RP_Transparent2D);
					pParticleGenerator2D->AddComponent(pGraphic, true);
				}
				else
				{
					bCancelled = true;
	...
	void MainFrame::OnDuplicate(wxCommandEvent& event)
	{
		...
		else
		{
			...
			u32 indexInParent = 0;
			for(u32 i=0; i<pEntity->GetParent()->GetChildCount(); ++i)
			{
