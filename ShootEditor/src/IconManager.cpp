...
namespace shoot
{
	...
	{	
		...
		m_pImageList->Add(wxArtProvider::GetBitmap(wxART_NORMAL_FILE));		
		m_IconMap[Camera::GetStaticClassName()] = wxBitmap(Camera_xpm);
		m_IconMap["EntityTemplate"] = wxArtProvider::GetBitmap(wxART_FOLDER_OPEN);
	...
	u32 IconManager::GetIconIndex(Object* pObject)
	{
		bool bFound = false;
		...
		if(bFound)
		{
			u32 index = 1+std::distance(m_IconMap.begin(), it);
			return index;
		}
		else
		{
	...
	wxBitmap IconManager::GetBitmap(IconType type)
	{
		wxBitmap bitmap;
