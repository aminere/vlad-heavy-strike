...
void wxVector2Property::RefreshChildren()
{
    if (GetChildCount())
	{	
		Vector2& vec = Vector2RefFromVariant(m_value);
...
void wxPointProperty::RefreshChildren()
{
    if (GetChildCount())
	{	
		Point& point = PointRefFromVariant(m_value);
...
void wxAABBox2DProperty::RefreshChildren()
{
    if (GetChildCount())
	{	
		AABBox2D& box = AABBox2DRefFromVariant(m_value);
...
{	
    AABBox2D& box = AABBox2DRefFromVariant(thisValue);
    switch ( childIndex )
    {
...
void wxAABBox3DProperty::RefreshChildren()
{
    if (GetChildCount())
	{	
		AABBox3D& box = AABBox3DRefFromVariant(m_value);
		Item(0)->SetValue(WXVARIANT(box.Min()));
		Item(1)->SetValue(WXVARIANT(box.Max()));
	}    
}
...
void wxSizeProperty::RefreshChildren()
{
    if (GetChildCount())
	{	
		Size& size = SizeRefFromVariant(m_value);
...
void wxRangeProperty::RefreshChildren()
{
    if (GetChildCount())
	{	
		Range& range = RangeRefFromVariant(m_value);
