...
namespace shoot
{
	...
	void TreeView::InsertEntity(Entity* pEntity)
	{
		if(!GetRootItem().IsOk())
		{
			...
			for(u32 i=0; i<pRoot->GetChildCount(); ++i)
			{
	...
	void TreeView::InsertEntity(Entity* pEntity, u32 index)
	{
		Entity* pParent = pEntity->GetParent();
		wxTreeItemId parentItemId = GetItemIDFromEntity(GetRootItem(), pParent);
		SHOOT_ASSERT(parentItemId.IsOk(), "Trying to insert entity into an unexisting tree item");
	...
	Vector3 TreeView::OnPreParentChange(Entity* pEntity)
	{
		...
		{
			vAbsPos = pEntity3D->GetTransformationMatrix().GetTranslation();
		}
		else if(Entity2D* pEntity2D = DYNAMIC_CAST(pEntity, Entity2D))
		{
			vAbsPos = pEntity2D->GetTransformationMatrix().GetTranslation();
		}
		return vAbsPos;
	}
	...
	void TreeView::OnPostParentChange(const Vector3& vPosition, Entity* pEntity)
	{
		...
		{
			pEntity2D->SetAbsolutePosition(Vector2(vPosition.X, vPosition.Y));
		}
	}
	...
	void TreeView::OnSelChanged(wxTreeEvent& event)
	{
		...
		if(itemId.IsOk())
		{
			ItemData* pData = static_cast<ItemData*>(GetItemData(itemId));
			pSelectedEntity = pData->m_Entity.Get();
		}
		...
		if(destItem.IsOk())
		{
			...
			{	
				...
				else
				{
					...
					for(u32 destIndex = 0; destIndex < pDestParent->GetChildCount(); ++destIndex)
					{
