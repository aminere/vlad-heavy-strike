...
namespace shoot
{
	...
	void NetworkManager::SendData(const char* strHostName, u32 port, u8* pData, u32 size, NetResultCallback* pResultCallback)
	{
		...
		int sendTimeOut = setsockopt(operation.m_SocketID, SOL_SOCKET, SO_SNDTIMEO, (const char*)&msec, sizeof(u32));
#else
		...
		if((operation.m_SocketID >= 0) && (rcvTimeOut == 0) && (sendTimeOut == 0))
		{
			...
			operation.m_pData = snew u8[size];
	...
	//! remove HTTP markers
	void NetworkManager::RemoveHTTPMarkers(const std::vector<u8>& dataIn, std::vector<u8>& dataOut)
	{
		bool bFoundCRLF = false;
		const s32 maxHeaderChars = 10;
		s32 i = 0;
		s32 copyOffset = 0;
		while(1 && i<s32(dataIn.size())-1)
		{
			if(!bFoundCRLF)
			{
			...
			else
			{
				...
				{
					s32 beforeCRLF = i-1;
					for(s32 l=0; l<maxHeaderChars && IsHexadecimal(dataIn[beforeCRLF]); ++l, --beforeCRLF);					
