...
namespace shoot
{
	...
	bool Plane::IntersectWithRay(const Vector3& vRayStart, const Vector3& vRayDir, Vector3* pIntersection /*= 0*/, E_Classification* pClass /*= 0*/) const
	{
		...
		{
			f32 denominator = Normal.DotProduct(vRayDir);
	...
	Plane::E_Classification Plane::ClassifyPoint(const Vector3& vPoint, f32* pNominator /*= 0*/) const
	{
		f32 fNominator = Normal.DotProduct(vPoint) - D;
