...
namespace shoot
{
	...
	void UISlider::Serialize(PropertyStream& stream)
	{
		...
		stream.Serialize(PT_Range, "PositionRange", &m_PositionRange);
	}
	...
	void UISlider::Update()
	{
		...
		else if(pInputMgr->IsTouchPressed())
		{
			if(m_bStartDragging)
			{
				...
				f32 fRatio = (iconPos.X-m_PositionRange.Min)/(m_PositionRange.Max-m_PositionRange.Min);
	...
	void UISlider::SetRatio(f32 fRatio)
	{
		if(m_Selector.IsValid())
		{
			Vector2 iconPos = m_Selector->GetPosition();
	...
	{
		Vector3 vTranlation = m_Selector->GetTransformationMatrix().GetTranslation();
		AABBox2D bbox = m_Selector->GetBoundingBox() + Vector2(vTranlation.X, vTranlation.Y);
