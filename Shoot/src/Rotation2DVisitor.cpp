...
namespace shoot
{
	...
	void Rotation2DVisitor::Visit(Entity* pTarget)
	{
		...
		m_fCurrentTime = 0.0f;
		ESP<f32, PT_Float>::Compute<Entity2D>(m_Src, m_Dest, m_fSrcAngle, m_fDestAngle, m_p2DEntity, &Entity2D::GetRotation);
		super::Visit(pTarget);
	}
	...
	void Rotation2DVisitor::Update()
	{
		...
		{
			f32 angle = Math::Lerp(m_fSrcAngle, m_fDestAngle, m_fCurrentTime/m_fDuration);			
