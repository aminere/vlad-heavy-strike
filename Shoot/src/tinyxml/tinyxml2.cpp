...
{
    Clear();
	shoot::File* pFile = shoot::File::Create(filename, shoot::File::M_ReadBinary);    
	if ( pFile->Open() )
	{
		LoadFile( pFile );
		pFile->Close();
...
XMLError XMLDocument::LoadFile( shoot::File* pFile )
{
    Clear();

	pFile->SetOffset(0, shoot::File::OT_End);
	size_t size = pFile->GetOffset();	
...
{
   	shoot::File* pFile = shoot::File::Create(filename, shoot::File::M_Write);
	if ( pFile->Open() )
	{
		SaveFile(pFile, compact);
		pFile->Close();
	}
	else
	{
...
XMLError XMLDocument::SaveFile( shoot::File* pFile, bool compact )
{
...
XMLPrinter::XMLPrinter( shoot::File* pFile, bool compact, int depth ) :
