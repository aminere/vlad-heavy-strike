...
namespace shoot
{
	...
	void VertexBufferDX11::GraphicLoad()
	{
		...
		if(m_pIndices)
		{
			...
			bd.ByteWidth = sizeof(u16) * m_NumIndices;
	...
	void VertexBufferDX11::Draw()
	{
		...
		DirectX11Driver::Instance()->GetContext()->IASetPrimitiveTopology(topologies[m_ePrimitiveType]);
	...
	void VertexBufferDX11::Draw(const std::vector<Matrix44>& aWorldTransforms, const std::vector<Matrix44>& aTextureTransforms, Material* pMaterial)
	{
		static D3D_PRIMITIVE_TOPOLOGY topologies[] = {
			D3D11_PRIMITIVE_TOPOLOGY_POINTLIST,			
			D3D11_PRIMITIVE_TOPOLOGY_LINELIST,
