...
namespace shoot
{
	...
	void CameraVisitor::Serialize(PropertyStream& stream)
	{
		...
		stream.Serialize(PT_Link, "LookAtTarget", &m_LookAtTarget);		
		stream.Serialize(PT_Float, "LookAtDuration", &m_fLookAtDuration);
		stream.Serialize(PT_Float, "Duration", &m_fDuration);
		stream.Serialize(PT_Float, "MaxDistToTarget", &m_fMaxDistToTarget);
	...
	void CameraVisitor::Update()
	{
		...
		{
			Vector3 vLookAtDest = m_LookAtTarget->GetTransformationMatrix().GetTranslation();
			...
			{
				f32 fDist = (vLookAtDest-m_pCamera->GetPosition()).GetLength();
