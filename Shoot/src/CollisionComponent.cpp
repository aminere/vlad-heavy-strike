...
namespace shoot
{
	...
	const s32 g_iCollisionGroupValues[] =
	{
	...
	void CollisionComponent::Serialize(PropertyStream& stream)
	{
		...
		stream.Serialize(PT_Enum, "CollisionGroup", &m_eGroup, ENUM_PARAMS2(g_strCollisionGroupLiterals, g_iCollisionGroupValues));
		stream.Serialize(PT_Struct, "CollisionMask", &m_Mask);
		stream.Serialize(PT_UInt, "Priority", &m_Priority);
		stream.Serialize(PT_Float, "CollisionRadius", &m_fCollisionRadius);
	}
	...
	void CollisionComponent::Init()
	{
		...
		if(Context* pRoot = GetParent()->GetRootContext())
		{
...
#define SERIALIZE_GROUP(x) \
		...
		if(stream.Serialize(PT_Bool, "Group_"#x, &bGroup_##x)) Set(Group_##x, bGroup_##x);
