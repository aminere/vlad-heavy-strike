...
namespace shoot
{
	...
	void EnvironmentCollisionComponent::Init()
	{
		...
		SHOOT_ASSERT(m_pParent->IsA(Entity3D::TypeID), "Parent not of type Entity3D");
	}
	...
	void EnvironmentCollisionComponent::CheckCollisions(CollisionComponent* pOther)
	{
		...
		{
			Vector3 vPosition = static_cast<Entity3D*>(pRigidBody->GetParent())->GetTransformationMatrix().GetTranslation();
	...
	bool EnvironmentCollisionComponent::CheckCollision(const Vector3& vPosition, const Vector3& vVelocity, f32 fRadius, Vector3* pIntersection /*= NULL*/)
	{
		Vertex3D* pVertices = m_Mesh->GetSubMeshes()[0].m_VertexBuffer->GetVertices();
		u16* pIndices = m_Mesh->GetSubMeshes()[0].m_VertexBuffer->GetIndices();
		u32 numIndices = m_Mesh->GetSubMeshes()[0].m_VertexBuffer->GetNumIndices();
		const Matrix44& matrix = static_cast<Entity3D*>(m_pParent)->GetTransformationMatrix();
		...
		for(u32 i=0; i<numIndices; i += 3)
		{
			...
			if(eObjectClass == Plane::C_Front)
			{
				...
				f32 fMotionLength = vVelocity.GetLength();
				Vector3 normalizedVelocity = vVelocity / fMotionLength;
				f32 cosAngle = normalizedVelocity.DotProduct(plane.Normal);				
				if(cosAngle < 0.0f || m_bDynamic)
				{
					...
					else
					{
						...
						f32 fDistToSphereIntersection = -1.0f;
