...
namespace shoot
{
	...
	bool CollisionUtils::AABBox3DIntersectsWithSphere(const AABBox3D& box, const Vector3& sphereCenter, f32 sphereRadius)
	{
		f32 s, d = 0.0f;
	...
	bool CollisionUtils::SphereIntersectsWithSphere(const Vector3& sphere1Center, f32 sphere1Radius, const Vector3& sphere2Center, f32 sphere2Radius)
	{
		f32 distSquared = (sphere1Center-sphere2Center).GetLengthSquared();
		f32 radii = sphere1Radius+sphere2Radius;
	...
	bool CollisionUtils::AABBox3DFastRayIntersectionCheck(const AABBox3D& box, const Vector3& rayStart, const Vector3& rayEnd)
	{
		...
		f32 rayHalfLength = (rayEnd - rayStart).GetLength() / 2.0f;
		...
		f32 r = e.Y * Math::FAbs(rayDir.Z) + e.Z * Math::FAbs(rayDir.Y);
