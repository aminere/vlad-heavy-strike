...
namespace shoot
{
	...
	void Scale2DVisitor::Visit(Entity* pTarget)
	{
		...
		m_fCurrentTime = 0.0f;
		ESP<Vector2, PT_Vec2>::Compute<Entity2D>(m_Src, m_Dest, m_vSrcScale, m_vDestScale, m_p2DEntity, &Entity2D::GetScale);			
		super::Visit(pTarget);	
	}
	...
	void Scale2DVisitor::Update()
	{
		...
		{
			Vector2 scale = Math::Lerp(m_vSrcScale, m_vDestScale, m_fCurrentTime/m_fDuration);			
