...
namespace shoot
{
	...
	private:
		...
		void ReadPartitions(const std::string& strDirectory, std::vector<PartitionInfo>& aPartitions, s32 currentPartition = -1);

		//! returns the ShootFS file size
		u32 GetShootFSFileSize(const char* strPath) const;

		//! get ShootFS file data
		void GetShootFSFileData(const char* strPath, u8* pDataOut) const;
