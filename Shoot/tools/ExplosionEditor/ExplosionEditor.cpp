...
void ExplosionEditor::Show()
{
	...
	m_UIManager.AddPane(m_pPropertyList, wxAuiPaneInfo().Right().CaptionVisible(false).CloseButton(false));

	// create the status bar
	const u32 numFields = 4;
...
{	
	...
	if(MeshResource* pMesh = m_ExplosionVisitor->GetMesh())
	{
		...
		for(u32 i=0; i<subMeshes.size(); ++i)
		{
			...
			for(u32 j=0; j<pVB->GetNumVertices(); ++j)
			{
				pVB->GetVertices()[j].color = Color(1.0f, 1.0f, 1.0f, 0.5f);
			}
		}
...
//! sets the selection
void ExplosionEditor::SetSelection(s32 chunkIndex, s32 subMeshIndex)
{
...
void ExplosionEditor::OnOpen(wxCommandEvent& event)
{
	...
	{
		Object* pObject = ObjectManager::Instance()->Get(explosionVisitorPath.c_str());
